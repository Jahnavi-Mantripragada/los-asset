...
const buildLeadDetails = (lead) => ({
  ...
  mobile: lead.mobile || "Not captured", email: lead.email || "Not captured",
...
/* ══ RICH TEXT EDITOR ══ */
function RichTextEditor({ onChange, placeholder }) {
...
/* ══ SEND EMAIL PANEL — Full Featured ══ */
function SendEmailPanel({ form, onChange, onSubmit, onClose, leadEmail }) {
  ...
  return (
    <BottomRightPanel type="email" title="Compose Email" onClose={onClose}
      footer={<><button className="brp-cancel-btn" onClick={onClose}>Discard</button><button className="brp-submit-btn" onClick={onSubmit}><MailIcon /> Send / Open</button></>}>
      <div className="email-recipients-block">
        ...
        <div className="email-field-row">
          ...
          <select className="form-select email-from-select" value={form.from||"sales@losportal.com"} onChange={(e)=>onChange("from",e.target.value)}>
      ...
      <div className="form-group" style={{flex:1}}>
        <label>Message</label>
        <RichTextEditor onChange={(html)=>onChange("bodyHtml",html)} placeholder="Write your email message here…" />
      </div>
      <div className="form-group">
        <label>Template</label>
        <select className="form-select" value={form.template||""} onChange={(e) => {
          onChange("template", e.target.value);
          if (e.target.value === "followup") onChange("subject","Following up on your Home Loan Application");
          if (e.target.value === "docs") onChange("subject","Documents Required — Home Loan Application");
        }}>
          <option value="">Load a template…</option>
          <option value="followup">Follow-up Template</option>
          <option value="docs">Document Request</option>
...
function ActivityItem({ item, isLast }) {
  const cfgs = { call:{emoji:"📞",cls:"call",lbl:"Call"}, task:{emoji:"✅",cls:"task",lbl:"Task"}, email:{emoji:"✉️",cls:"email",lbl:"Email"}, status:{emoji:"🔄",cls:"status",lbl:"Update"}, note:{emoji:"📝",cls:"note",lbl:"Note"} };
...
function LeadDetailPage({ lead, onBack, onLogout, onConvertLead }) {
  ...
  const handleSendEmail = () => {
    const to=panelForm.to||""; const subject=panelForm.subject||""; const body=(panelForm.bodyHtml||"").replace(/<[^>]+>/g," ").trim();
  ...
  const handleDisqualify = () => {
    setLeadStatus("Disqualified"); setLeadData(p=>({...p,leadStage:"Disqualified"}));
    addActivity({type:"status",title:"Lead Disqualified",desc:`Reason: ${panelForm.reason||"Not specified"}`});
    setShowModal(null); setPanelForm({});
  };
  const handleConvert = () => {
  ...
  const handleStatusStep = (step) => {
    ...
    const prev=leadStatus; setLeadStatus(step); setLeadData(d=>({...d,leadStage:step}));
  ...
  const fp = (key, sectionId) => ({
    fieldKey:key, value:leadData[key]??"—", sectionId,
    sectionEditMode, sectionDraft,
    onSectionDraftChange:(k,v)=>setSectionDraft(p=>({...p,[k]:v})),
    editingField, onEdit:handleFieldEdit, onChange:handleFieldChange, onSave:handleFieldSave, onCancel:handleFieldCancel,
  });
  ...
  return (
    <div className="lead-detail-layout">
      {/* BACKDROP - page functional but slightly dimmed */}
        ...
        {activeTab==="activity"&&(
          <div className="activity-layout">
            <div className="activity-action-bar">
              ...
              <button className="aab-btn aab-email" onClick={()=>setShowPanel("email")}><MailIcon /> Send Email</button>
      ...
      {showPanel==="email" && <SendEmailPanel   form={panelForm} onChange={handlePanelChange} onSubmit={handleSendEmail}   onClose={closePanel} leadEmail={leadData.email} />}
